...
import { IMAGE_AI_TOOLS, getImageAiTool } from "@/lib/ai-center/tools/image-ai";
...
// ---------------------------------------------------------------------------
describe("Image AI tool definitions", () => {
  it("registers exactly the 10 required tools", () => {
    expect(IMAGE_AI_TOOLS.map((t) => t.slug).sort()).toEqual([...EXPECTED_SLUGS].sort());
    expect(IMAGE_AI_TOOLS).toHaveLength(10);
  });

  it("every tool has a unique routeSegment and at least one required field", () => {
    const segments = IMAGE_AI_TOOLS.map((t) => t.routeSegment);
    expect(new Set(segments).size).toBe(segments.length);
    for (const tool of IMAGE_AI_TOOLS) {
      expect(tool.fields.some((f) => f.required)).toBe(true);
    }
  });

  it("getImageAiTool resolves by routeSegment and returns undefined for unknown segments", () => {
    expect(getImageAiTool("image-prompt")?.slug).toBe("image-prompt-generator");
    expect(getImageAiTool("style-transfer")?.slug).toBe("style-transfer-prompt");
    expect(getImageAiTool("no-existe")).toBeUndefined();
  });

  it("findToolDefinition (shared, cross-platform) resolves every Image AI slug too", () => {
    for (const slug of EXPECTED_SLUGS) {
      expect(findToolDefinition(slug)?.slug).toBe(slug);
    }
  });

  it("slugs never collide with any other category's tool slugs, including the pre-existing 'prompts' category", () => {
...
// ---------------------------------------------------------------------------
// Prompts válidos: solo texto, agnósticos de motor, sin hechos inventados
// ---------------------------------------------------------------------------
describe("Image AI prompts are valid, engine-agnostic, and never assume unprovided facts", () => {
  ...
  it("no Image AI prompt reuses another category's system prompt exact text", () => {
    const imagePrompts = IMAGE_AI_TOOLS.map((t) => t.buildSystemPrompt("ctx"));
  ...
  it("every tool explicitly states it never generates an image, only a text prompt", () => {
    for (const tool of IMAGE_AI_TOOLS) {
      expect(tool.buildSystemPrompt("ctx")).toMatch(/No generas imágenes/);
    }
  });

  it("every tool explicitly refuses to invent unprovided facts about the subject", () => {
    for (const tool of IMAGE_AI_TOOLS) {
      expect(tool.buildSystemPrompt("ctx")).toMatch(/No inventes/);
    }
  });

  it("every tool asks for engine-agnostic, natural-language output that works across multiple named engines", () => {
    for (const tool of IMAGE_AI_TOOLS) {
      const prompt = tool.buildSystemPrompt("ctx");
      expect(prompt).toMatch(/Midjourney|DALL·E|Stable Diffusion|Flux/);
      expect(prompt).toMatch(/lenguaje descriptivo natural/);
    }
  });

  it("the character and style-transfer tools treat user-supplied descriptions as data, never inventing beyond them", () => {
    const character = IMAGE_AI_TOOLS.find((t) => t.slug === "character-prompt-generator")!;
    const styleTransfer = IMAGE_AI_TOOLS.find((t) => t.slug === "style-transfer-prompt")!;
    expect(
      character.buildUserPrompt({ descripcionPersonaje: "Guerrera con armadura azul", estiloArtistico: "fantasía", idioma: "es" })
    ).toMatch(/usa solo esto, no añadas rasgos nuevos/);
    expect(
      styleTransfer.buildUserPrompt({ contenidoBase: "Un gato sentado en una silla", estiloObjetivo: "acuarela", idioma: "es" })
    ).toMatch(/usa solo esto, no inventes más/);
  });

  it("the product photography tool never invents brand/product facts beyond the given description", () => {
    const product = IMAGE_AI_TOOLS.find((t) => t.slug === "product-photography-prompt")!;
    expect(
      product.buildUserPrompt({ producto: "Botella de agua reutilizable", entorno: "x", iluminacion: "x", idioma: "es" })
    ).toMatch(/no inventes marca ni características/);
  });
});
...
describe("AI Center integration: same architecture as every other tool category", () => {
  it("a new 'Image AI' category exposes all 10 tools as 'available' with a real href matching each routeSegment", () => {
    const category = AI_CENTER_CATEGORIES.find((c) => c.slug === "image-ai")!;
    expect(category).toBeDefined();
    expect(category.label).toBe("Image AI");
    for (const slug of EXPECTED_SLUGS) {
      ...
      expect(registryTool.href?.("proj1")).toBe(`/dashboard/proj1/ai-center/image-ai/${definition.routeSegment}`);
    }
  });

  it("the pre-existing empty 'Imágenes' category and coming-soon 'Prompts' category are completely untouched", () => {
    const imagenes = AI_CENTER_CATEGORIES.find((c) => c.slug === "imagenes")!;
    expect(imagenes.label).toBe("Imágenes");
    expect(imagenes.tools).toEqual([]);

    const prompts = AI_CENTER_CATEGORIES.find((c) => c.slug === "prompts")!;
    expect(prompts.label).toBe("Prompts");
    expect(prompts.tools.map((t) => t.slug).sort()).toEqual(
      ["prompts-midjourney", "prompts-flux", "prompts-imagen", "prompts-runway", "prompts-veo", "prompts-kling"].sort()
    );
    expect(prompts.tools.every((t) => t.status === "coming-soon")).toBe(true);
  });

  it("findAiTool resolves category/label for every Image AI slug", () => {
    for (const slug of EXPECTED_SLUGS) {
      const tool = findAiTool(slug);
      expect(tool?.categorySlug).toBe("image-ai");
      expect(tool?.categoryLabel).toBe("Image AI");
    }
  });

  it("one dynamic route serves all 10 tools — no per-tool page files were created", () => {
    const dynamicPage = "src/app/(dashboard)/dashboard/[projectId]/ai-center/image-ai/[tool]/page.tsx";
    expect(existsSync(path.join(ROOT, dynamicPage))).toBe(true);
    for (const tool of IMAGE_AI_TOOLS) {
      const perToolPage = `src/app/(dashboard)/dashboard/[projectId]/ai-center/image-ai/${tool.routeSegment}/page.tsx`;
  ...
  it("the dynamic page reuses AiGenerationForm — the exact same generic engine every other category uses, no second form", () => {
    const page = read("src/app/(dashboard)/dashboard/[projectId]/ai-center/image-ai/[tool]/page.tsx");
  ...
  it("no Sidebar entry was added — reached only through the AI Center hub, exactly like every other AI category", () => {
    ...
    expect(allLabels).not.toContain("Image AI");
  ...
  it("guest navigation is untouched", () => {
    ...
    expect(labels).not.toContain("Logo Prompt Generator");
  });
});
...
describe("Workspace integration: same save action, no second history", () => {
  ...
  it("no second save action or history table was created for Image AI", () => {
    expect(existsSync(path.join(ROOT, "src/server/actions/image-ai.ts"))).toBe(false);
    expect(existsSync(path.join(ROOT, "src/server/actions/ai-center-image-ai.ts"))).toBe(false);
  });

  it("UniversalResultViewer was not modified — its existing mediaKind design already accommodates a future image provider", () => {
    const viewer = read("src/components/workspace/universal-result-viewer.tsx");
    expect(viewer).toMatch(/"text" \| "image" \| "pdf" \| "audio" \| "video"/);
    expect(viewer).not.toMatch(/image-ai|imageAi/i);
  });
});
...
describe("Chat IA automatically detects the new tools via the AI Center registry — no code changes to Chat IA", () => {
  it("listRoutableTools (used by the intent classifier) already includes all 10 Image AI tools", () => {
  ...
  it("listToolDefinitions includes every category's tools together, from one shared registry, with no duplicates", () => {
    ...
    for (const tool of [...OTHER_CATEGORY_TOOLS, ...IMAGE_AI_TOOLS]) {
      expect(all).toContain(tool.slug);
    }
  });

  it("Chat IA's panel and the intent router were NOT modified by this phase — no Image-AI-specific code inside them", () => {
    const panel = read("src/components/chat/chat-panel.tsx");
    const router = read("src/lib/chat/intent-router.ts");
    expect(panel).not.toMatch(/image-ai|image prompt/i);
    expect(router).not.toMatch(/image-ai|image prompt/i);
  });
...
// ---------------------------------------------------------------------------
// Seguridad / sin APIs externas
// ---------------------------------------------------------------------------
describe("Security and no external API integration", () => {
  it("the project layout's membership guard still runs for every ai-center/image-ai/* request", () => {
  ...
  it("Image AI results never leak contentType/resultKind from client input — always resolved server-side from the tool definition", () => {
    const action = read("src/server/actions/ai-center-tools.ts");
  ...
  it("no file in this phase imports an image-generation SDK/API or makes a network call — only comments may name them to explain what's excluded", () => {
    const files = [
      "src/lib/ai-center/tools/image-ai.ts",
      "src/lib/ai-center/tools/image-ai-prompts.ts",
  ...
  it("package.json has no new image-generation dependency added", () => {
    const pkg = JSON.parse(read("package.json"));
    const deps = { ...pkg.dependencies, ...pkg.devDependencies };
    for (const forbidden of ["openai", "replicate", "@fal-ai/serverless-client", "stability-client"]) {
...
describe("No duplication introduced by this phase", () => {
  it("AiGenerationForm is defined in exactly one place, reused by every AI category route", () => {
    ...
    expect(existsSync(path.join(ROOT, "src/components/ai-center/generation/image-ai-generation-form.tsx"))).toBe(false);
  });
  ...
  it("no new AI engine was introduced — engine.ts is unchanged and still the only generation entry point", () => {
    ...
    expect(existsSync(path.join(ROOT, "src/lib/ai/local/image-engine.ts"))).toBe(false);
  });
  ...
  it("every previous category is untouched: same tool counts, same routes", () => {
    ...
    for (const routeDir of ["youtube", "instagram", "social-media", "blog-seo", "email-marketing", "tiktok-ai", "facebook-ai", "linkedin-ai"]) {
      expect(
