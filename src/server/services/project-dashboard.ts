...
export async function getProjectDashboardData(projectId: string) {
  ...
  const [
    ...
    activeCampaigns,
    upcomingPosts,
    legacyActiveAutomations,
    activeWorkflowAutomations,
    monitorsWithIssues,
    brokenLinks,
    ...
    prisma.workflowAutomation.count({ where: { projectId, status: "ACTIVE" } }),
  ...
  return {
    ...
    activeCampaigns,
    upcomingPosts,
    // Sums the legacy Automation model (unchanged, out-of-phase system) with the new Automation Center's ACTIVE WorkflowAutomation rows — both link to the same /automations route, which now renders only the new hub.
    activeAutomations: legacyActiveAutomations + activeWorkflowAutomations,
    monitorsWithIssues,
    brokenLinks,
    connectedIntegrations,
    aiUsageThisMonth: {
