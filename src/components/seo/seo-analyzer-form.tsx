...
import { runSeoAnalysisAction, type SeoFormState } from "@/server/actions/seo";
...
export function SeoAnalyzerForm({ projectId }: { projectId: string }) {
  const action = runSeoAnalysisAction.bind(null, projectId);
      ...
      {state.result ? (
        <div className="space-y-4 rounded-lg border p-4">
          <div className="flex items-center gap-3">
            <span className="text-3xl font-semibold">{state.result.score}</span>
            <div className="flex-1">
              <Progress value={state.result.score} />
              <p className="mt-1 text-xs text-muted-foreground">
                Puntuación calculada con reglas deterministas. No es una garantía de posicionamiento en buscadores.
              </p>
            </div>
          </div>
