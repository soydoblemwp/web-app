...
import { saveGeneratedContentAdaptationAction } from "@/server/actions/content";
...
export function ContentAdapterForm({
  projectId,
  brandContextText,
}: {
  projectId: string;
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const system = buildContentAdapterSystemPrompt(brandContextText);
    ...
    setResult(text);
    setSaving(true);
    const saveResult = await saveGeneratedContentAdaptationAction({ projectId, targetPlatform, body: text, language });
    setSaving(false);
    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button type="submit" disabled={busy}>
          {saving ? "Guardando..." : busy ? "Adaptando..." : "Adaptar contenido"}
        </Button>
